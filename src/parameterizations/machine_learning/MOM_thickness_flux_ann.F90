...
subroutine thickness_flux_ann_init(Time, G, GV, US, param_file, diag,CS)
    ...
    call get_param(param_file, mdl, "ann_coeff", CS%ann_coeff, &
                       "Coefficient to multiply the ANN output by", default=1.0, units="nondim")
    call get_param(param_file, mdl, "ann_window", CS%ann_window, &
                          "Number of horizontal grid points to use in the ANN window", default=3)

    ! Setup ann for thickness fluxes
    call get_param(param_file, mdl, "thickness_flux_ann_num_layers", CS%ann_cs%num_layers, &
                       "Number of ANN layers for thickness flux", default=4)
    call get_param(param_file, mdl, "thickness_flux_ann_params_file", CS%ann_cs%NNfile, &
                       "Thickness_flux ANN parameters netcdf input", default="thickness_flux_ann_params.nc")
    call ann_init(CS%ann_cs)
